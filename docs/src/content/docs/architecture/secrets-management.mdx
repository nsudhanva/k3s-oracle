...
description: OCI Vault integration for secure secrets storage
---
...
1. **Dynamic Group**: `k3s-nodes-dg` groups instances by tag (`Project = k3s-oracle-free`).
2. **Policy**: `k3s-secrets-read-policy` allows the dynamic group to `read secret-bundles` in the compartment.
3. **ClusterSecretStore**: Configured with `auth: instancePrincipal: {}`.
